...
class RobotTrajectories(object):
    ...
    def generate_data(self,n_samples):
        data = []
        t = np.linspace(0, 2 * np.pi, self.seq_length // self.n_periods)
        for _ in range(n_samples):
            ...
            amp1 = self.random_state.rand(self.n_batch, self.n_sines) * 2.5 * 30
            periods = self.random_state.rand(self.n_batch, self.n_sines) * .7 + .3
            omega1 = np.sin(t[..., None, None] / periods[None, ...] + phase_motor1[None, ...]) * amp1[None, ...]
            omega1 = (omega1 / (omega1.max(0) - omega1.min(0)) * 40.).sum(-1)
            ...
            # y = np.cumsum(y, 1) * dt_step
                 ...
                 # Find the current min and max of x and y
            ...
            omega0 = omega0.T[:, :self.seq_length]
            omega1 = omega1.T[:, :self.seq_length]
        ...
        #  x = (np.cos(phi0) + np.cos(phi1)).T * (self.random_state.rand(self.n_batch, self.seq_length) * 100 - 2)
        #     y = (np.sin(phi0) + np.sin(phi1)).T * (self.random_state.rand(self.n_batch, self.seq_length) * 100 - 2)
